...
export const AppProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    ...
    const initializeCloudSync = async () => {
      ...
      try {
        ...
        // Strictly respect user directive: If neither had central state, DO NOT upload local storage!
        if (!dataApplied) {
          console.log('[Database Boot] No central cloud or server records found. Strictly preserving database integrity without uploading local storage.');
          setDatabaseLoadSource(connected ? 'cloud' : 'offline');
        }
  ...
  useEffect(() => {
    // CRITICAL SECURITY & INTEGRITY GUARD:
    ...
    // 4. No user mutation has occurred in this active session (!hasUserMutatedDataRef.current)
    // This strictly prevents stale or unwanted local storage data from ever being uploaded!
    if (
      isInitialDatabaseLoading ||
      !isCloudInitializedRef.current ||
      isApplyingCloudDataRef.current ||
      !hasUserMutatedDataRef.current
    ) {
      return;
    }
    ...
    FirebaseService.syncToCloudDebounced(
      ...
      (lastSyncedAt) => {
        ...
        flashUploadSuccess();
        finishProgressTracking(true, 'تمت المزامنة وحفظ التعديلات 100%', expenses.length);
      },
      () => {
  ...
  const addExpense = async (expenseData: Omit<Expense, 'id' | 'fingerprintTime' | 'updatedAt' | 'synced' | 'status' | 'accountantApproval' | 'managementApproval'>) => {
    ...
    recordOperationStatus(
      'إضافة مصروف جديد',
      `تم قيد وحفظ المصروف رقم (${newExpense.id}) بمبلغ ${newExpense.amount.toLocaleString()} ${settings.currencySymbol} لمشروع ${newExpense.projectName}`,
      { savedLocally: true, synced: effectiveOnline, details: `المشروع: ${newExpense.projectName}` }
    );
  ...
  const updateExpense = async (id: string, expenseData: Partial<Expense>) => {
    ...
    if (updated) {
      recordOperationStatus(
        'تعديل وحفظ المصروف',
        `تم حفظ تعديلات المصروف (${id}) محلياً ومزامنتها بنجاح`,
  ...
  const deleteExpense = async (id: string) => {
    ...
    if (deleted) {
      ...
      recordOperationStatus(
        'حذف المصروف',
        `تم حذف المصروف رقم (${id}) وحفظ التعديلات في النظام`,
        { savedLocally: true, synced: effectiveOnline }
      );
    }
  ...
  const bulkDeleteExpenses = async (ids: string[]) => {
    ...
    if (authorizedIdsArray.length > 0) {
      ...
      recordOperationStatus(
        'حذف جماعي للمصروفات',
        `تم حذف (${count}) سند بنجاح وحفظ الحالة محلياً`,
        { savedLocally: true, synced: effectiveOnline }
      );
    }
  ...
  const approveWorkflowStage = async (
    ...
    if (success) {
      recordOperationStatus(
        ...
        `تم اعتماد الفاتورة (${expenseId}) بمرحلة (${targetStage.title}) وحفظ الحالة محلياً ومزامنتها`,
        { savedLocally: true, synced: effectiveOnline }
      );
  ...
  const postToExternalERP = async (id: string, erpSystemName: string, erpReferenceNumber: string, notes?: string) => {
    ...
    if (updated) {
      recordOperationStatus(
        ...
        `تم ترحيل المصروف (${id}) على نظام (${erpSystemName}) بالقيد (${erpReferenceNumber})`,
        { savedLocally: true, synced: effectiveOnline }
      );
      addNotification(
  ...
  const rejectExpense = async (id: string, reason: string) => {
      ...
      prev.map(exp => {
        if (exp.id === id) {
          ...
          return {
            ...exp,
            status: 'مرفوض',
            supervisorApproval: 'مرفوض',
            projectManagerApproval: 'مرفوض',
            accountantApproval: 'مرفوض',
            managementApproval: 'مرفوض',
    ...
    if (rejected) {
      recordOperationStatus(
        'رفض المصروف',
        `تم رفض الفاتورة رقم (${id}) وحفظ التعديل محلياً وسحابياً`,
        { savedLocally: true, synced: effectiveOnline }
      );
      addNotification(
        'تم رفض المصروف',
  ...
  const addCustody = async (custodyData: Omit<CustodyRecord, 'id' | 'synced'>) => {
    ...
    recordOperationStatus(
      'صرف عهدة مالية',
      `تم تسليم وصرف دفعة عهدة بمبلغ ${newCustody.amount.toLocaleString()} ${settings.currencySymbol} إلى (${newCustody.supervisorName}) وحفظ السند بنجاح`,
      { savedLocally: true, synced: effectiveOnline }
    );
  ...
  const updateCustody = async (id: string, custodyData: Partial<CustodyRecord>) => {
    ...
    setCustodies(prev =>
      prev.map(c => {
        if (c.id === id) {
          updated = true;
          return { ...c, ...custodyData, updatedAt: nowIso, synced: effectiveOnline, savedLocally: true, lastSavedAt: nowIso };
        }
        return c;
      })
    );
    if (updated) {
      recordOperationStatus(
        'تحديث سند العهدة',
        `تم حفظ تعديلات سند العهدة رقم (${id}) محلياً ومزامنتها`,
        { savedLocally: true, synced: effectiveOnline }
      );
  ...
  const deleteCustody = async (id: string) => {
    ...
    if (deleted) {
      ...
      recordOperationStatus(
        'حذف سند العهدة',
        `تم حذف سند العهدة (${id}) وحفظ التعديل بنجاح`,
        { savedLocally: true, synced: effectiveOnline }
      );
    }
    return deleted;
  ...
  const addProject = async (projectData: Omit<Project, 'id' | 'createdAt'>) => {
    ...
    recordOperationStatus(
      'إضافة مشروع جديد',
      `تمت إضافة وحفظ المشروع "${newProject.name}" في النظام بنجاح ومزامنته`,
      { savedLocally: true, synced: effectiveOnline }
    );
  ...
  const updateProject = async (id: string, projectData: Partial<Project>) => {
    ...
    if (updated) {
      recordOperationStatus(
        'تحديث المشروع',
        `تم حفظ تعديلات المشروع (${projectData.name || id}) بنجاح`,
        { savedLocally: true, synced: effectiveOnline }
      );
    }
  ...
  const deleteProject = async (id: string) => {
    ...
    if (deleted) {
      ...
      recordOperationStatus(
        'حذف المشروع',
        `تم حذف المشروع (${id}) من النظام وتحديث التعيينات`,
        { savedLocally: true, synced: effectiveOnline }
      );
  ...
  const addUser = async (userData: Omit<User, 'id'>) => {
    ...
    // Synchronously persist to StorageService
    StorageService.saveUsers(nextList.length > 0 ? nextList : [...users, newUser]);
    ...
    recordOperationStatus('إضافة مستخدم', `تمت إضافة العضو (${newUser.name}) بنجاح`, { savedLocally: true, synced: effectiveOnline });
    return newUser;
  };
  ...
  const updateUser = async (id: string, userData: Partial<User>) => {
    ...
    // Synchronously persist updated users immediately
    if (nextUsersList.length > 0) {
      StorageService.saveUsers(nextUsersList);
    }
    ...
    if (updated && targetUser) {
      recordOperationStatus('تحديث مستخدم', `تم تحديث بيانات وصلاحيات العضو (${targetUser.name}) بنجاح`, { savedLocally: true, synced: effectiveOnline });
    }
  ...
  const deleteUser = async (id: string) => {
    ...
    if (deleted) {
      ...
      recordOperationStatus('حذف مستخدم', `تم حذف العضو (${targetUser.name}) بنجاح`, { savedLocally: true, synced: effectiveOnline });
    }
    return deleted;
  };
